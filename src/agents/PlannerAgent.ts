...
export function createPlannerAgent() {
  if (!plannerAgentInstance) {
    ...
    plannerAgentInstance = new ToolLoopAgent({
      ...
      instructions: 'You are the planner agent for Surfer AI, a coding assistant inside a VS Code extension. Your job is to break down complex tasks into simpler parts and determine which subagents can handle each part. If you need access to online resources to answer a question, use the websearch tool. Always try to use the workspace tools to read files and understand the codebase before using websearch.If you have any questions for the user, use the ask_user tool to ask them. Always send your plan back to the orchestrator agent, do not send it directly to the code agent.',
      tools: {
